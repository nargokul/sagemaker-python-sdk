...
class TestSmdPrepare(unittest.TestCase):
    ...
    @patch("shutil.copy2")
    def test_prepare_for_smd_with_inference_spec(
        self, mock_copy, mock_capture, mock_gen_key, mock_hash
    ):
    ...
    def test_prepare_for_smd_with_custom_orchestrator(
        self, mock_copy, mock_capture, mock_gen_key, mock_hash, mock_rename
    ):
    ...
    @patch("sagemaker.serve.model_server.smd.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_smd_with_shared_libs(
        self, mock_copy, mock_capture, mock_gen_key, mock_hash
    ):
